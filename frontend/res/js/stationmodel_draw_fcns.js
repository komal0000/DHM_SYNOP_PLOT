...
function draw_wind(w_speed, w_dir, obj) {
    var wind_speed = parseInt(w_speed);
    var wind_direction = parseInt(w_dir);
    var flag_width = 20;
    var flag_length = 50;
    ...
    if (wind_direction > 359) {
        wind_direction -= 360;
      console.log("Error: Wind direction out of range (direction was reset to between 0 and 359 degrees).");
    w_dir = wind_direction;
    }

    if (wind_speed > 200) {
        wind_speed = 200;
       console.log("Error: Wind speed out of range (reset to 200 knots).");
        w_speed= 200;
    }
