...
import { upperair_temperatureLayers } from './layers.js';

export function synopObservation(data) {
  function createWindBarb(speed, direction, my_temp, my_dewpt, cloud_state, cloud_high_type, cloud_low_type, cloud_mid_type, pressure, pressure_change, visibility, weather_code, type, stationId,height) {
  ...
  .lt(8.84, -8.84);
    ...
    function drawPressure(obj, pressure_mb) {
      const pressure = pressure_mb ;
      const encoded = pressure;
  ...
  const layerTypes = [
    ...
    { title: 'Pressure Height', type: 'pressure' },
  ...
  layerTypes.forEach(({ title, type }) => {
    ...
    upperair_temperatureLayers.getLayers().push(layer);
  });
}